...
import { fetchFxHistory, FxHistory } from '../lib/fxHistory'
import { useSettings, TranslationKey, Theme, Language, Currency } from '../lib/settings'
import { calculateTWR, calculateMWR, annualizeReturn, calculateCorrelation, netCashFlowBetween, deriveCashFlows, CashFlow, PortfolioSnapshot, ValuationPoint } from '../lib/returns'
...
function Dashboard({ onLogout }: DashboardProps) {
  ...
  const fetchData = useCallback(async (silent = false) => {
    ...
    try {
      const [assetsResult, historyResult, rateResult, txResult, tradesResult] = await Promise.allSettled([
        ...
        fetchAllRows<PortfolioHistory>('portfolio_history', 'created_at', true),
      ...
      if (historyResult.status === 'fulfilled' && !historyResult.value.error) {
        ...
        throw historyResult.value.error
      }
  ...
  // Najwcześniejszy przepływ gotówki wyznacza początek potrzebnego zakresu kursów.
  const firstFlowDate = useMemo(() => {
    ...
    for (const rt of realizedTrades) {
      const day = rt.buy_date.slice(0, 10)
      if (!earliest || day < earliest) earliest = day
    }
    return earliest
  }, [buyTransactions, realizedTrades])

  useEffect(() => {
    if (!firstFlowDate) return
    let cancelled = false
    const today = new Date().toISOString().slice(0, 10)
    fetchFxHistory(firstFlowDate, today).then((h) => {
      if (!cancelled && h) setFxHistory(h)
    })
    return () => {
      cancelled = true
    }
  ...
  // Przepływy gotówki przeliczamy kursem z dnia transakcji — tak samo, jak cron zapisał
  // wartość portfela do portfolio_history. Dzisiejszy kurs dawałby w TWR/MWR różnicę
  // kursową udającą zysk z rynku. Bez danych historycznych wracamy do kursu bieżącego.
  const fxRateAt = useCallback(
    (currency: string, date: string) => {
      if (currency === 'PLN') return 1
      return fxRateFor(currency, fxHistory?.rateFor(date.slice(0, 10)) ?? fxRates)
    },
    [fxHistory, fxRates]
  )
  ...
  // Dzienna zmiana portfela: żywa wartość vs poprzedni dzień, skorygowana o wpłaty/wypłaty
  const dailyChange = useMemo(() => {
    ...
    const cashFlows: CashFlow[] = [
      ...buyTransactions.map(tx => ({
        ...
        amount: Number(tx.price) * Number(tx.quantity) * fxRateAt(tx.currency, tx.date),
      })),
      ...realizedTrades.map(rt => ({
        date: rt.sell_date.slice(0, 10),
        amount: -(Number(rt.sell_price) * Number(rt.quantity) * fxRateAt(rt.currency, rt.sell_date)),
      })),
    ]
    const netCf = netCashFlowBetween(cashFlows, prev.day, today)
    const change = totalValue - prev.val - netCf
    const base = prev.val
    const pct = base > 0 ? (change / base) * 100 : 0
    return { value: change, percent: pct }
  }, [history, totalValue, buyTransactions, realizedTrades, fxRateAt])
  ...
  const portfolioReturns = useMemo(() => {
    ...
    // Jedna migawka na dzień (ostatnia z danego dnia)
    const byDay = new Map<string, PortfolioHistory>()
    for (const h of history) byDay.set(h.created_at.slice(0, 10), h)
    const days = Array.from(byDay.keys()).sort()
    const today = new Date().toISOString().slice(0, 10)

    // Koszt nabycia w walucie natywnej narastająco na dzień — potrzebny tylko do
    // oszacowania dryfu kursowego, więc kilkudniowa nieścisłość dat nie ma znaczenia.
    const nativeCostByDay = new Map<string, Record<string, number>>()
    {
      const acc: Record<string, number> = {}
      let bi = 0
      let ri = 0
      for (const day of [...days, today]) {
        while (bi < buyTransactions.length && buyTransactions[bi].date.slice(0, 10) <= day) {
          const tx = buyTransactions[bi++]
          acc[tx.currency] = (acc[tx.currency] ?? 0) + Number(tx.price) * Number(tx.quantity)
        }
        while (ri < realizedTrades.length && realizedTrades[ri].sell_date.slice(0, 10) <= day) {
          const rt = realizedTrades[ri++]
          acc[rt.currency] = (acc[rt.currency] ?? 0) - Number(rt.buy_price) * Number(rt.quantity)
        }
        nativeCostByDay.set(day, { ...acc })
      }
    }

    const fxDriftBetween = (prevDay: string, day: string): number => {
      const native = nativeCostByDay.get(prevDay)
      if (!native) return 0
      let drift = 0
      for (const [currency, amount] of Object.entries(native)) {
        if (currency === 'PLN' || amount === 0) continue
        drift += amount * (fxRateAt(currency, day) - fxRateAt(currency, prevDay))
      }
      return drift
    }

    const snapshots: PortfolioSnapshot[] = days.map((day, i) => {
      const h = byDay.get(day)!
      return {
        date: day,
        value: h.total_value,
        cost: h.total_value - (h.total_pnl ?? 0),
        realized: h.realized_pnl_total ?? 0,
        fxCostDrift: i === 0 ? 0 : fxDriftBetween(days[i - 1], day),
      }
    })

    // Dzisiejszy stan z żywych cen dopina się na koniec szeregu.
    if (days[days.length - 1] !== today) {
      ...
      snapshots.push({
        ...
        value: totalValue,
        cost: totalCost,
        realized: snapshots[snapshots.length - 1]?.realized ?? 0,
        fxCostDrift: fxDriftBetween(prevDay, today),
      })
    }

    const valuations: ValuationPoint[] = snapshots.map(s => ({ date: s.date, value: s.value }))
    const daysElapsed = Math.round(
      (new Date(valuations[valuations.length - 1].date).getTime() -
        new Date(valuations[0].date).getTime()) / 86400000
    )

    // TWR i korelacja: przepływy z migawek, bo tylko one są zgodne w czasie z wyceną.
    const snapshotFlows = deriveCashFlows(snapshots)

    // MWR odpowiada na pytanie „ile zarobił mój kapitał", więc tu liczą się realne
    // daty transakcji, a nie moment, w którym pozycja trafiła do aplikacji.
    const investorFlows: CashFlow[] = [
      ...buyTransactions.map(tx => ({
        date: tx.date.slice(0, 10),
        amount: Number(tx.price) * Number(tx.quantity) * fxRateAt(tx.currency, tx.date),
      })),
    ...
    const twr = calculateTWR(valuations, snapshotFlows)
    const mwr = calculateMWR(investorFlows, totalValue, today)
    ...
    // Korelacja dziennych zwrotów portfela z S&P 500 (z korektą CF jak w TWR)
    ...
    const correlation = calculateCorrelation(valuations, sp500Valuations, snapshotFlows)

    return { twr, mwr, twrAnnualized, correlation }
  }, [history, buyTransactions, realizedTrades, fxRateAt, totalValue, totalCost])
  ...
  const lineChartData: ChartPoint[] = useMemo(() => {
    ...
    const deduplicatedHistory = Array.from(byDay.values()).sort((a, b) =>
      ...
      for (const day of sortedDays) {
        while (bi < buyTransactions.length && buyTransactions[bi].date <= day) {
          ...
          if (shouldIncludeTicker(tx.ticker)) {
            acc += tx.price * tx.quantity * fxRateAt(tx.currency, tx.date)
          }
        }
        while (ri < realizedTrades.length && realizedTrades[ri].sell_date <= day) {
          const rt = realizedTrades[ri++]
          if (shouldIncludeTicker(rt.ticker)) {
            acc -= rt.buy_price * rt.quantity * fxRateAt(rt.currency, rt.buy_date)
          }
        }
  ...
  }, [history, filterType, chartView, chartPercent, showBenchmark, showRealized, showTradeMarkers, buyTransactions, realizedTrades, fxRateAt])

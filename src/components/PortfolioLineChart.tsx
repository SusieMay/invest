...
/** Domenę Y: trochę poniżej min i powyżej max w bieżącym zakresie (nie pinujemy do 0). */
function paddedYDomain(
  values: number[],
  padRatio = 0.08
...
function filterByTimeRange(data: ChartPoint[], range: TimeRange): ChartPoint[] {
  ...
  // Kotwica = ostatni dzień w danych (nie zegar lokalny) — spójne z UTC day-key w Dashboard.
  const tip = dayKey(data[data.length - 1].date)
  const [ty, tm, td] = tip.split('-').map(Number)
  const tipUtc = Date.UTC(ty, tm - 1, td)

  let cutoffMs: number
  switch (range) {
    case '5d':
      cutoffMs = tipUtc - 5 * 86400000
      break
    case '1m': {
      const d = new Date(tipUtc)
  ...
  const filtered = data.filter(d => dayKey(d.date) >= cutoff)
...
function PortfolioLineChart({ data, chartType, timeRange, scaleType, showPercent, showBenchmark, showInvested, showTradeMarkers }: Props) {
  const memoizedData = useMemo(() => {
    ...
    for (const d of chartData) {
      if (typeof d.value === 'number') yValues.push(d.value)
      if (hasInvestedData && typeof (d as { invested?: number }).invested === 'number') {
        yValues.push((d as { invested: number }).invested)
      }
      if (hasBenchmarkData && typeof (d as { benchmark?: number }).benchmark === 'number') {
        yValues.push((d as { benchmark: number }).benchmark)
      }
    }
    const yDomain = useLog ? (['auto', 'auto'] as ['auto', 'auto']) : paddedYDomain(yValues)

    return { chartData, hasPnl, isPercentMode, hasBenchmarkData, hasInvestedData, useLog, lineColor, lineColorDot, label, offset, tradeIndices, yDomain }
  ...
  const { chartData, hasPnl, isPercentMode, hasBenchmarkData, hasInvestedData, useLog, lineColor, lineColorDot, label, offset, tradeIndices, yDomain } = memoizedData
    ...
    <ResponsiveContainer width="100%" height={420}>
      <ComposedChart data={chartData} margin={{ top: 5, right: 10, left: 5, bottom: 5 }}>
        ...
        <XAxis
          ...
          tick={{ fill: 'rgb(var(--c-stone-500))', fontSize: 10 }}
          tickLine={false}
          minTickGap={28}

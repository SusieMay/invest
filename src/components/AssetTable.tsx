...
type SortField = 'ticker' | 'quantity' | 'avgPrice' | 'currentPrice' | 'positionValue' | 'pnl' | 'pnlPercent'
  ...
  const sortedAssets = useMemo(() => {
    return [...assets].filter(a => !pendingDelete.has(a.id)).sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'pnlPercent': {
          const pctA = a.average_price > 0 ? (((a.current_price ?? a.average_price) - a.average_price) / a.average_price) * 100 : 0
          const pctB = b.average_price > 0 ? (((b.current_price ?? b.average_price) - b.average_price) / b.average_price) * 100 : 0
    ...
    try {
      ...
      if (mode === 'sell') {
        ...
        await supabase.from('realized_trades').insert({
          ...
          sell_date: today,
          profit_pln: toPLN(profitNative, cur, fxRates),
        })
      }
        ...
        <table className="w-full text-sm min-w-[760px]">
          <thead>
            <tr className="text-stone-500 text-left border-perf">
              ...
              <th className="pb-3 px-2 font-bold text-right cursor-pointer select-none" {...headerProps('pnlPercent')}>Zysk %<SortIcon field="pnlPercent" /></th>
              ...
              return (
                ...
                >
                  ...
                  <td className="py-3.5 px-2 text-right">
                    <div className={isPositive ? 'text-[#2D6A4F]' : 'text-[#A83232]'}>
                      <NativeAmount
                        value={pnl}
                  ...
                  <td className="py-3.5 px-2 text-right">
                    <span className={`font-mono font-medium ${isPositive ? 'text-[#2D6A4F]' : 'text-[#A83232]'}`}>

...
import { PortfolioHistory, Transaction, RealizedTrade } from '../types'
import { FxRates, fxRateFor } from '../lib/currency'
import { CashFlow, netCashFlowBetween } from '../lib/returns'
 ...
 * czerwone w dni na minus. Dzienny wynik = zmiana wartości skorygowana o
 * wpłaty/wypłaty (jak TWR i karta „Zmiana dzienna”). Dla dzisiejszego dnia
 * używana jest żywa wartość (liveTotalValue).
 */
function DailyReturnsCalendar({
  history,
  liveTotalValue,
  buyTransactions = [],
  realizedTrades = [],
  fxRates = { usdPln: 4, eurPln: 4.3, krwPln: 0.003 },
}: Props) {
  const { returns, months } = useMemo(() => {
    ...
    for (const day of days) {
      ...
      if (prevDay != null && prevValue != null) {
        const netCf = netCashFlowBetween(cashFlows, prevDay, day)
        returns.set(day, value - prevValue - netCf)
      }
      prevDay = day
      prevValue = value
    }
  ...
  }, [history, liveTotalValue, buyTransactions, realizedTrades, fxRates])

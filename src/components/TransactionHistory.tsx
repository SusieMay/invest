...
type SortField = 'date' | 'ticker' | 'platform' | 'profit' | 'cagr'
...
function TransactionHistory({ exchangeRate, eurRate, krwRate = 0.003, assets, onDataChange }: Props) {
  ...
  const enriched = transactions.map(tx => {
    ...
    const profitPLN = tx.type === 'buy' && matchedSell != null
      ? toPLN((matchedSell.price - tx.price) * tx.quantity, tx.currency, fxRates)
      : null
    const isRealized = tx.type === 'buy' && matchedSell != null
    const valuePLN = toPLN(tx.quantity * tx.price, tx.currency, fxRates)

    return { ...tx, matchedSell, matchedBuy, exitPrice, holdDays, cagr, roi, profitPLN, isRealized, valuePLN }
  })
  ...
  const sorted = [...filtered].sort((a, b) => {
    ...
    switch (sortField) {
      ...
      case 'profit':   return dir * ((a.profitPLN ?? -Infinity) - (b.profitPLN ?? -Infinity))
      ...
      if (type === 'buy') {
        ...
        if (existing) {
          ...
          await supabase.from('realized_trades').insert({
            ...
            sell_date: date, profit_pln: profitNative * rate,
          })
          const newQty = existing.quantity - qty
          if (newQty <= 0) {
    ...
    try {
      ...
      if (existing) {
        ...
        await supabase.from('realized_trades').insert({
          ...
          sell_date: closeDate, profit_pln: toPLN(profitNative, buyTx.currency, fxRates),
        })
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-stone-500 text-left border-perf">
                    ...
                    <th className="pb-3 px-2 font-bold text-right cursor-pointer select-none hover:text-[#33332D]" onClick={() => handleSort('profit')}>Zysk / Strata <SortIcon field="profit" /></th>
                      ...
                      <React.Fragment key={tx.id}>
                        <tr className={`border-perf transition-colors ${isExpanded ? 'bg-stone-200/60' : 'hover:bg-stone-200/50'}`}>
                          ...
                          <td className="py-3 px-2 text-right whitespace-nowrap">
                            {tx.profitPLN != null ? (
                              <span className={`font-mono font-bold text-xs ${tx.profitPLN >= 0 ? 'text-[#2D6A4F]' : 'text-[#A83232]'}`}>
                                {tx.profitPLN >= 0 ? '+' : ''}{formatPLN(tx.profitPLN)}
                              </span>
                            ) : (
                              <span className="text-stone-400 text-xs">—</span>
                            )}
